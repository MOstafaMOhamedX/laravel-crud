...
use MOstafaMOhamedX\Crud\Utils\RouteUtils;
use MOstafaMOhamedX\Crud\Utils\SchemaUtils;
use MOstafaMOhamedX\Crud\Utils\TextUtils;

class ViewGen extends BaseGen {
    public function __construct(public array $tables, public array $aliases = [], public ?string $routePrefix = NULL, public bool $singleForm = FALSE) {
    ...
    public function genForm(string $template, string $path, array $replacements, bool $edit) {
        $input = function ($f) use ($edit, $path) {
            ...
            $default = !empty($f->default) ? " ?? '$f->default'" : NULL;
            $val = $edit ? sprintf('@old(\'%s\', %s)%s', $f->id, $f->type === 'json' ? "json_encode(\$_var_->{$f->id})" : "\$_var_->{$f->id}", $default) : sprintf('@old(\'%s\')%s', $f->id, $default);
